...
<body>
            ...
            <main class="col-md-9 col-lg-10 ms-auto">
                <div class="p-4">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h1 class="fw-bold mb-0">Daftar Kategori</h1>
                        ...
                        <div class="card-body">
                                ...
                                <table class="table table-hover align-middle">
                                    ...
                                    <tbody>
                                        <?php foreach ($categories as $category):
                                            $catId = $category->id ?? $category['id'];
                                            $catNama = $category->nama ?? $category['nama'];
                                            $catSlug = $category->slug ?? $category['slug'];
                                            $catActive = $category->is_active ?? $category['is_active'] ?? true;
                                            $productCount = $category->product_count ?? 0;
                                        ?>
                                            <tr>
                                                <td>#<?= esc($catId) ?></td>
                                                <td><?= esc($catNama) ?></td>
                                                <td><code><?= esc($catSlug) ?></code></td>
                                                <td>
                                                    <?php if ($catActive): ?>
                                                ...
                                                <td><?= esc($productCount) ?> produk</td>
                                                <td>
                                                    <a href="<?= base_url('/admin/kategori/' . $catId . '/edit') ?>" class="btn btn-sm btn-outline-primary">
                                                        <span class="material-symbols-outlined">edit</span>
                                                    </a>
                                                    <a href="<?= base_url('/admin/kategori/' . $catId . '/delete') ?>" class="btn btn-sm btn-outline-danger"
                            ...
                            <?php else: ?>
                                <div class="text-center py-4">
                                    <i class="material-symbols-outlined text-muted" style="font-size: 48px;">category</i>
    ...
    <script>
        var baseURL = '<?= base_url('/admin/kategori/') ?>';

        function deleteCategory(catId) {
            if (confirm('Yakin ingin menghapus kategori ini? Semua produk terkait akan diubah menjadi tidak aktif.')) {
                fetch(baseURL + catId + '/delete', {
                    method: 'POST',
                    headers: {
