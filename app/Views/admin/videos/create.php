...
<body>
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-md-9 col-lg-10 ms-auto">
                <div class="p-4">
                    <h1 class="fw-bold mb-4">Tambah Video</h1>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form action="<?= base_url('/admin/video') ?>" method="POST">
                                <?= csrf_field() ?>

                                <div class="mb-3">
                                    <label for="judul" class="form-label">Judul Video *</label>
                                    <input type="text" class="form-control" id="judul" name="judul" required placeholder="Judul video">
                                </div>

                                <div class="mb-3">
                                    <label for="youtube_url" class="form-label">URL YouTube *</label>
                                    <input type="url" class="form-control" id="youtube_url" name="youtube_url" required placeholder="https://www.youtube.com/watch?v=example">
    ...
    <script>
        document.getElementById('youtube_url')?.addEventListener('change', function() {
            var url = this.value;
            var idMatch = url.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|v\/))([^?&]+)/);
            if (idMatch && idMatch[1]) {
                var thumbnail = document.createElement('img');
                thumbnail.src = 'https://img.youtube.com/vi/' + idMatch[1] + '/mqdefault.jpg';
                thumbnail.style.maxWidth = '100%';
                thumbnail.style.maxHeight = '200px';
                thumbnail.style.objectFit = 'cover';
                thumbnail.alt = 'Preview Video';

                var previewDiv = document.getElementById('youtube-preview');
                if (previewDiv) {
